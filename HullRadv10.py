...
Version 9 (2023)
    Add intrinsic viscosity and sedimentation non-ideality constant
        (Many thanks to Jack Corriea for encouragement)
Version 10 (2024)
    Optimize sedimentation non-ideality constant and 
       add diffusion non-ideality constant
    Add total hydration in g water/g protein
    Add specific volume of hydrated molecule 
    Add excluded volume second virial coefficient
...
Output looks like:
 ...
 8rat.pdb
  ...
  M               :        13690     g/mol
  v_bar           :        0.710     mL/g
  Ro(Anhydrous)   :        15.68     Angstroms
  Rg(Anhydrous)   :        14.35     Angstroms
  Dmax            :        45.01     Angstroms
  Axial Ratio     :         1.31
  f/fo            :         1.21
  Dt              :       1.13e-06   cm^2/s
  R(Translation)  :        18.99     Angstroms
  s20,w           :       1.85e-13   sec
  Int. Viscosity  :         3.58     ml/g 
  Total Hydration :         0.54     g/g    
  Spc Vol Hyd Prot:         1.25     mL/g     
  ks(Non-ideal)   :         6.82     ml/g 
  kd(Non-ideal)   :         4.01     ml/g 
  Bex(2nd virial) :         5.05     ml/g (Excluded volume)
  Asphericity     :         0.14     (from Gyration Tensor)
  Dr              :       1.91e+07   s^-1
  R(Rotation)     :        20.36     Angstroms
  tauC            :         8.74     ns (from R_rotation)

To write out the unified side chain psuedo-atom model uncomment the line
near the bottom labeled, # Write out model (~line 1520)
...
USAGE = """
python3 HullRadV10.py [PDBcode].pdb
    or 
python3 HullRadV10.py [PDBcode].cif
"""
...
# Mass is minus water (from SEDNTERP database, 2022)
...
# vbar is 0.60224(V/M)
#
# Note that recent experimental vbar measurements differ
# Hellman et al., Eur Biophys J 39:389, 2010
# But values below work within HullRad
#
# V = (vbar*M)/0.60224 or V = 0.91325*M
#
#          Mass         Volume   vbar      ND
NA_data = {
...
GL_data = {
 ...
 'FRU':  ( 180.16,       188.4,  0.622,   0.00), # FRUCTOSE
...
#
# Choose your vbar depending on the MW of your PEG
#  vbar = 0.8576 * MW^-0.0032561
# Then correct the Volume
#
# Luo, Z., and Zhang, G. J. Phys. Chem. B 113:12462, 2009
#
# See also Lepori, L., and Mollica, V. J. Polymer Sci. 16:1123, 1978 
#
# Note: Uses only the 'C1' atoms of a PEG PDB file, ignores other atoms
#
# V = (vbar*M)/0.60224
#          Mass         Volume   vbar      ND
PEG_data = {
 'PEG':  ( 44.05,       60.7,  0.830,   0.00), # POLYETHYLENE GLYCOL (same as PEO)
 'ETH':  ( 44.05,       60.7,  0.830,   0.00), # POLYETHYLENE GLYCOL (same as PEO)
 'OTH':  ( 44.05,       60.7,  0.830,   0.00), # POLYETHYLENE GLYCOL (same as PEO)
 'PEO':  ( 44.05,       60.7,  0.830,   0.00)  # POLYETHYLENE GLYCOL (same as PEO)
}
...
def model_from_pdb(file):
    ...
    dt_rec = []
    # Initial PEO/PEG atoms from PDB file
    pg_rec = []
            ...
            if resname != 'TIP' and resname != 'HOH':
                ...
                # Detergents
                  ...
                  line[13:14] == 'O' or line[14:15] == 'O'):
                    dt_rec.append(line)
                # Polyethylene glycol
                # Note: Uses only the 'C1' atoms of polyethylene glycol,
                #    ignores other atoms
                elif resname in PEG_data and (line[13:15] == 'C1'):
                    pg_rec.append(line)
    ...
    # Put PEG atoms into initial array
    num_atoms =  len(pg_rec)
    pg_array = [['X' for j in range(8)] for i in range(num_atoms)]
    for row in range(len(pg_array)):
        pg_array[row][0] = row
        pg_array[row][1] = (pg_rec[row][11:16]).strip()
        pg_array[row][2] = (pg_rec[row][17:20]).strip()
        pg_array[row][3] = (pg_rec[row][20:22]).strip()
        pg_array[row][4] = (pg_rec[row][22:26]).strip()
        pg_array[row][5] = (pg_rec[row][30:38]).strip()
        pg_array[row][6] = (pg_rec[row][38:46]).strip()
        pg_array[row][7] = (pg_rec[row][46:54]).strip()

    # Add detergent atoms into model array
    for row in range(len(pg_array)):
        model_array.append(pg_array[row])
...
#       print(gyration_tensor)
    ...
    for row in range(len(model_array)):
        ...
        # DNA, RNA, Saccharides, Detergents
            ...
            if rnum != last_rnum:
                ...
                for rec in  PEG_data.items():
                    if rnam == rec[0]:
                        aa_mass, vol_aa, vbar_aa, sc_rad_aa = PEG_data[rnam]
                        numerator += (aa_mass * vbar_aa)
                        prot_mol_mass += aa_mass
                        last_rnum = rnum
                        PG = PG +1
    ...
    # numerator += (aa_mass * vbar_aa) from above
...
#   print('ellipb = ', b)
    ...
    tot_hydration = tot_water_mass/prot_mol_mass

    if tot_hydration > 1.0:
        vol_hull = 0.9 * vol_hull
    vol_hyd_hull = vol_hull + vol_shell_wat

    tot_water_vol = vol_hyd_hull - (vol_prot)
    tot_water_mass = (tot_water_vol/29.8) * 18.01 * 0.998213
    tot_hydration = tot_water_mass/prot_mol_mass
    ...
    # Correct this to 0.0100167 (Rukes and Dooley 2020 IAPWS G5-01 Int. Assoc. 
    #   Properties Water Steam 1-7)
    eta = 0.0100167 # poise 20degC
    # Correct this to 0.998213 (Patek et al. 2009 J. Phys. Chemical Ref Data 38:21)
    rho = 0.998213 # g/ml water density 20degC
    ...
    #  where, numerator += (aa_mass * vbar_aa)
    # Using v_bar of water = 1.0
    vbar_hyd_prot = (numerator + tot_water_mass) / prot_mol_mass
    # which is the same as vs_vp = vbar_prot + tot_hydration (See Philo, EBJ 2023)
#   print(('  Hydrated Volume :    %9.0f     A^3 ' % (vol_hyd_hull)))
#   print(('  Anhydrous Volume:    %9.0f     A^3 ' % (vol_prot)))
    ...
    return s,Dt,Dr,vbar_prot,Rht,ffo_hyd_P,prot_mol_mass,Ro,Rhr,int_vis,a_b_ratio, \
           Ft,Rg,Dmax,tauC,asphr,int_vis,tot_hydration,vbar_hyd_prot,ks, \
           TanfordBex,kd,AA,NA,GL,DT,PG,useNumpy
...
def process_file(file_path):
    """
    Process a single file (pdb, ent, or cif).
    Return a dictionary with fields that can go into a Polars DataFrame.
    """
    filename = Path(file_path).stem 
    extension = Path(file_path).suffix[1:].lower()

    if extension in ('pdb', 'ent'):
        all_atm_rec, num_MG, num_MN, model_array = model_from_pdb(file_path)
    elif extension == 'cif':
        all_atm_rec, num_MG, num_MN, model_array = model_from_cif(file_path)
    else:
        # Unsupported extension in this function.
        # You may want to raise an exception or return None to skip
        return None

    (
        s, Dt, Dr, vbar_prot, Rht, ffo_hyd_P, M, Ro, Rhr, int_vis,
        a_b_ratio, Ft, Rg, Dmax, tauC, asphr, int_vis2,
        tot_hydration, vbar_hyd_prot, ks, TanfordBex, kd,
        AA, NA, GL, DT_, PG, useNumpy
    ...
    return {
        ...
        "Rg_Anhydrous_Ang": Rg,
        "Dmax_Ang": Dmax,
        "Axial_Ratio": a_b_ratio,
        "f_fo": ffo_hyd_P,
        "Dt_cm2_s": Dt,
        "R_Translation_Ang": Rht,
        "s20_w_sec": s,
        "Int_Vis_ml_g": int_vis,
        "Total_Hydration_g_g": tot_hydration,
        "Spc_Vol_Hyd_Prot_mL_g": vbar_hyd_prot,
        "ks_ml_g": ks,
        "kd_ml_g": kd,
        "Bex_ml_g": TanfordBex,
        "Asphericity": asphr if useNumpy else None,
        "Dr_s_inv": Dr,
        "R_Rotation_Ang": Rhr,
        "tauC_ns": tauC
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Process a folder of PDB-like files in parallel and output a Polars DataFrame."
    )
    parser.add_argument(
        "--input",
        help="Path to folder containing .pdb, .ent or .cif files."
    )
    parser.add_argument(
        "-o", "--output_csv",
        default="results.csv",
        help="Name of the output CSV file (default: results.csv)."
    )
    parser.add_argument(
        "-n", "--num_workers",
        type=int,
    ...
    folder = args.input
    all_files = sorted(os.listdir(folder)) # for testing
    valid_ext = (".pdb", ".ent", ".cif")
    file_paths = [os.path.join(folder, f) for f in all_files 
                  if f.lower().endswith(valid_ext)]

    if not file_paths:
        print("No valid pdb/ent/cif files found in the input folder.")
        sys.exit(1)
    ...
    # Filter out any None results (if you skip unsupported files)
    results = [r for r in results if r is not None]

    # Create Polars DataFrame
    df = pl.DataFrame(results)

    # Write to CSV (you can also write parquet, etc.)
    df.write_csv(f"{args.output_csv}.tsv",separator = "\t")
    end = time.perf_counter()
    duration = end - start
    print(f"Done. Results written to {args.output_csv} in {duration:.2f} seconds.")
